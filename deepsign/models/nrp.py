...
class LBLNRP(tx.Model):
    ...
    def __init__(self,
        ...
        with tf.name_scope("run"):
            ...
            # features for all words with shape [ri_layer.shape[0], embed_dim]
            run_features = tx.Linear(ri_layer, embed_dim, logit_init, shared_weights, name="logits", bias=False)

            # dot product of f_predicted and all features
            run_logits = tx.Linear(f_prediction,
                                   n_units=vocab_size,
                                   shared_weights=tf.transpose(run_features.tensor))
