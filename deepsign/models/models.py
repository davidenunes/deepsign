...
class NRP(tx.Model):
    def __init__(self,
        ...
        if self.loss_inputs is None:
            # loss inputs are the positive + negative indices by this order
            loss_inputs = tx.Input(self.ri_act, dtype=tf.int64, name="ri_labels_loss")
            self.loss_inputs = loss_inputs

        if self.eval_inputs is None:
            eval_inputs = tx.Input(n_units=self.ri_act, dtype=tf.int64, name="ri_labels_eval")
            self.eval_inputs = eval_inputs
        ...
        # lookup layer
        # batch_size = None uses dynamic lookup
        ...
        if self.nce:
            ...
            sampled_values = tf.nn.uniform_candidate_sampler(true_classes=self.loss_inputs.tensor,
                                                             num_true=self.ri_act,
                                                             num_sampled=self.nce_samples,
                                                             unique=True,
                                                             range_max=self.ri_dim * 2)
            ...
            train_loss = tf.nn.nce_loss(weights=tf.transpose(train_logits.weights),
                                        ...
                                        labels=self.loss_inputs.tensor,
                                        num_sampled=self.nce_samples,
                                        num_classes=self.ri_dim * 2,
                                        num_true=self.ri_act,
                                        sampled_values=sampled_values)
        else:
            # the problem with binary cross entropy is that it computes it
            ...
            # TODO test binary cross entropy with reduce mean accross last dimension
            # TODO perplexity has to be computed per feature/class basis
            # TODO perplexity for binary case is maxed out at 2 because we have two classes on average
            # it's right...
            # I can adjust it to match the multi-classe perplexity of vocab size
            # and display that
            # TODO adjust it and show it
            train_loss = tf.reduce_mean(tx.binary_cross_entropy(self.loss_inputs.tensor, train_logits.tensor), axis=-1)

        # ===============================================
        # EVAL GRAPH
        # ===============================================
        one_hot = tx.dense_one_hot(column_indices=self.eval_inputs.tensor, num_cols=self.ri_dim * 2)
        eval_loss = tf.reduce_mean(tx.binary_cross_entropy(one_hot, run_logits.tensor), axis=-1)
        eval_tensors = tf.reduce_mean(eval_loss)
